...
export interface RawEntry { start_time: string | null; end_time: string | null; course_title: string | null; teacher_name: string | null; }
export type RawData = Record<string, Record<string, RawEntry[]>>;
export interface Slot { start: string; end: string; course: string; teacher: string; }

export function parseHM(t: string | null | undefined): number | null {
  if(!t) return null; const [h,m] = t.split(':').map(Number); if(Number.isNaN(h)||Number.isNaN(m)) return null; return h*60+m;
}

export function formatDelta(mins: number | null): string {
  if(mins==null) return ''; if(mins <=0) return 'Now'; const h=Math.floor(mins/60); const m=mins%60; const parts:string[]=[]; if(h) parts.push(h+'h'); if(m) parts.push(m+'m'); return parts.join(' ');
}

export function roomSlotsForDay(rawData: RawData, day: string, room: string): Slot[] {
...
export function isRoomFreeAt(rawData: RawData, day: string, room: string, minutes: number): OccupancyStatus {
  ...
  for (let i=0;i<slots.length;i++) {
    ...
    if (minutes >= start && minutes < end) {
      ...
      return { free: false, nextStart: null, current: s, currentEnds: end, immediateContinuation, nextAfterCurrentStart };
    }
    if (start > minutes) { nextStart = start; break; }
  }
  return { free: true, nextStart };
}

export function computeSoonFree(list: { room: string; status: OccupancyStatus }[], minutes: number, soonThreshold: number) {
