 ...
 */
export function createFormWithDefaults(
  ...
  const filteredCustomFields = customFields.filter(
    (customField) =>
      !defaultRequiredFields.some(
        (defaultField) => defaultField.name === customField.name
      )
  );
 ...
 */
export function validateCustomFields(fields: FormField[]): {
  isValid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  fields.forEach((field, index) => {
    ...
    if (!/^[a-zA-Z][a-zA-Z0-9_]*$/.test(field.name)) {
      errors.push(
        `Invalid field name "${field.name}" at index ${index}. Names must start with a letter and contain only letters, numbers, and underscores.`
      );
    }
    ...
    if (!validTypes.includes(field.type)) {
      errors.push(
        `Invalid field type "${field.type}" for field "${field.name}". Must be one of: ${validTypes.join(', ')}`
      );
    }
  });
...
export const FORM_FIELD_PRESETS = {
  ...
  standard: [
    ...
    },
    {
      ...
      type: 'textarea',
      required: false,
      placeholder: 'Tell us about your relevant experience or expectations...',
    },
  ] as FormField[],
  ...
  hackathon: [
    ...
    },
    {
      ...
      type: 'text',
      required: false,
      placeholder: 'Enter your team name (leave blank if individual)',
    },
    {
      name: 'skillLevel',
    ...
    },
    {
      ...
      type: 'textarea',
      required: false,
      placeholder: "List technologies you're comfortable with...",
    },
    {
      name: 'projectIdea',
  ...
  conference: [
    ...
    },
    {
      ...
      type: 'textarea',
      required: false,
      placeholder: 'Which sessions or topics are you most interested in?',
    },
    {
      name: 'networking',
  ...
  competition: [
    ...
    },
    {
      ...
      required: false,
      placeholder:
        'List other team members (name, email) if participating as a team...',
    },
    {
      name: 'experience',
      label: 'Relevant Experience',
    ...
    },
    {
      ...
      type: 'select',
      required: true,
      options: ['Individual', 'Team (2-3 members)', 'Team (4+ members)'],
    },
  ] as FormField[],
};
...
export function createRegistrationForm(
  ...
  const { eventId, customFields, customColors = {} } = options;

  const fields =
    customFields ||
    FORM_FIELD_PRESETS[eventType] ||
    FORM_FIELD_PRESETS.standard;
  const colorKey = (
    eventType in COLOR_SCHEMES ? eventType : 'workshop'
